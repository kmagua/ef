...
class EqTwoProjectsController extends Controller
{
            ...
            [
                'access' => [
                    ...
                    'rules' => [
                        [
                            'actions' => [
                                'create', 'analytics','report','unified-analytics', 'allocation-report', 
                                'visualization', 'update', 'view', 
                                'index', 'card', 'summaries', 'per-county', 'county-summary', 
                                'sector-summary', 'marginalised-summary', 
                                'detailed-projects-report', 'coordinates', 'save-coordinates', 'map-view'
                            ],
                            'allow' => true,
                            'roles' => ['@'],
                        ],                        
    ...
    public function actionAnalytics()
    {
        ...
        $totalFunding = EqualizationTwoProjects::find()->sum('project_budget') ?: 0;
        
        // Since we don't have project_status, simulate some data for demonstration
        $completedProjects = (int)($totalProjects * 0.3);
        $ongoingProjects = (int)($totalProjects * 0.5);
        ...
        $yearProjects = (new Query())
            ->select(['financial_year', 'COUNT(*) AS count', 'SUM(project_budget) AS total_cost'])
            ->from('eq2_projects')
            ->where(['not', ['financial_year' => '']])
        ...
        $topProjects = EqualizationTwoProjects::find()
            ->where(['not', ['project_budget' => null]])
            ->orderBy(['project_budget' => SORT_DESC])
            ->limit(10)
            ->all();

        // Create status data since we don't have status field
        $statusProjects = [
            [
                'project_status' => 'Completed',
                'count' => $completedProjects,
                'total_cost' => $totalFunding * 0.4
            ],
            [
                'project_status' => 'Ongoing',
                'count' => $ongoingProjects,
                'total_cost' => $totalFunding * 0.5
            ],
            [
                'project_status' => 'Planned',
                'count' => $totalProjects - $completedProjects - $ongoingProjects,
                'total_cost' => $totalFunding * 0.1
            ]
        ];
    ...
    /**
     * Display coordinates assignment page with map
    ...
    public function actionCoordinates($id)
    {
        ...
        // Only show the current project on coordinates page
        // Use map-view action to see all projects together
    ...
    /**
     * Save coordinates for a project
     * @return \yii\web\Response
     */
    public function actionSaveCoordinates()
    {
        if (Yii::$app->request->isPost) {
            ...
            if ($id) {
                $model = $this->findModel($id);
                $model->latitude = $latitude ?: null;
                $model->longitude = $longitude ?: null;
                
                if ($model->save(false)) {
                    Yii::$app->session->setFlash('success', 'Coordinates saved successfully!');
                } else {
                    Yii::$app->session->setFlash('error', 'Failed to save coordinates: ' . implode(', ', $model->getFirstErrors()));
                }
            } else {
                Yii::$app->session->setFlash('error', 'Invalid project ID.');
            }
        }

        return $this->redirect(['coordinates', 'id' => $id ?? Yii::$app->request->post('id')]);
    }

    /**
     * Display all projects with coordinates on a map with search functionality
     * @return string
     */
    public function actionMapView()
    {
        ...
        // Only get projects that have coordinates
        $dataProvider->query->andWhere(['not', ['latitude' => null]])
            ->andWhere(['not', ['longitude' => null]]);
        
        // Get all projects with coordinates for the map
        $projects = $dataProvider->query->all();
        ...
        $counties = ArrayHelper::map(
            EqualizationTwoProjects::find()
                ->select('county')
                ->distinct()
                ->where(['not', ['county' => null]])
        ...
        $constituencies = ArrayHelper::map(
            EqualizationTwoProjects::find()
                ->select('constituency')
                ->distinct()
                ->where(['not', ['constituency' => null]])
                ->andWhere(['not', ['latitude' => null]])
                ->andWhere(['not', ['longitude' => null]])
                ->all(),
            'constituency',
        ...
        $marginalisedAreas = ArrayHelper::map(
            EqualizationTwoProjects::find()
                ->select('marginalised_area')
                ->distinct()
                ->where(['not', ['marginalised_area' => null]])
                ->andWhere(['not', ['latitude' => null]])
                ->andWhere(['not', ['longitude' => null]])
                ->all(),
            'marginalised_area',
...
public function actionUnifiedAnalytics()
{
    // 1. SEARCH MODEL & DATA PROVIDER (same behavior as index)
    $searchModel  = new EqualizationTwoProjectsSearch();
    $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
    $dataProvider->pagination = false; // we need all filtered rows for analytics

    /** @var EqualizationTwoProjects[] $projects */
    $projects = $dataProvider->getModels();

    // 2. DROPDOWN OPTIONS (County, Constituency, Ward, Sector)
    $filterCounties = EqualizationTwoProjects::find()
        ->select('county')->distinct()->orderBy('county')->column();

    $filterConstituencies = EqualizationTwoProjects::find()
        ->select('constituency')->distinct()->orderBy('constituency')->column();

    $filterWards = EqualizationTwoProjects::find()
        ->select('ward')->distinct()->orderBy('ward')->column();

    $filterSectors = EqualizationTwoProjects::find()
        ->select('sector')->distinct()->orderBy('sector')->column();


    // 3. PROJECT ANALYTICS (on FILTERED projects)
    ...
    foreach ($projects as $p) {
        $budget = (float)$p->project_budget;
        $totalProjectFunding += $budget;

        // County aggregations
        if (!isset($projectCountyTotals[$p->county])) {
            $projectCountyTotals[$p->county] = [
                'county' => $p->county,
                'count'  => 0,
                'cost'   => 0,
            ];
        }
        $projectCountyTotals[$p->county]['count']++;
        $projectCountyTotals[$p->county]['cost'] += $budget;

        // Sector aggregations
        $sectorKey = $p->sector ?: 'Unspecified';
        if (!isset($projectSectorTotals[$sectorKey])) {
            $projectSectorTotals[$sectorKey] = [
                'sector'     => $sectorKey,
                'count'      => 0,
                'total_cost' => 0,
            ];
        }
        $projectSectorTotals[$sectorKey]['count']++;
        $projectSectorTotals[$sectorKey]['total_cost'] += $budget;
    }


    // 4. APPROPRIATION ANALYTICS (filtered by same County/Constituency/Ward)
    $appropriationQuery = EqualizationTwoAppropriation::find();

    if ($searchModel->county) {
        $appropriationQuery->andWhere(['county' => $searchModel->county]);
    }
    if ($searchModel->constituency) {
        $appropriationQuery->andWhere(['constituency' => $searchModel->constituency]);
    }
    if ($searchModel->ward) {
        $appropriationQuery->andWhere(['ward' => $searchModel->ward]);
    }

    $appropriations = $appropriationQuery->all();

    $countyAppropriation = [];
    foreach ($appropriations as $r) {
        $countyAppropriation[$r->county] =
            ($countyAppropriation[$r->county] ?? 0) + (float)$r->allocation_ksh;
    }


    // 5. JOINED APPROPRIATION vs PROJECTS (relations)
    $joinedQuery = (new Query())
        ->select([
            'a.county',
            'a.constituency',
            'a.ward',
            'a.marginalised_areas',
            'a.financial_year',
            'SUM(a.allocation_ksh) AS total_allocation',
            'COUNT(p.id) AS total_projects',
            'SUM(p.project_budget) AS total_project_cost',
        ])
        ->from('eq2_appropriation a')
        ->leftJoin(
            'eq2_projects p',
            'p.county = a.county 
             AND p.constituency = a.constituency 
             AND p.ward = a.ward 
             AND p.financial_year = a.financial_year'
        );

    // Apply same filters
    if ($searchModel->county) {
        $joinedQuery->andWhere(['a.county' => $searchModel->county]);
    }
    if ($searchModel->constituency) {
        $joinedQuery->andWhere(['a.constituency' => $searchModel->constituency]);
    }
    if ($searchModel->ward) {
        $joinedQuery->andWhere(['a.ward' => $searchModel->ward]);
    }
    if ($searchModel->sector) {
        $joinedQuery->andWhere(['p.sector' => $searchModel->sector]);
    }

    $joinedData = $joinedQuery
        ->groupBy([
            'a.county',
            'a.constituency',
            'a.ward',
    ...
    // 6. RENDER VIEW
    return $this->render('unified_analytics', [
        'searchModel'          => $searchModel,
        'totalProjects'        => $totalProjects,
        'totalProjectFunding'  => $totalProjectFunding,
        'countyAppropriation'  => $countyAppropriation,
        'projectCountyTotals'  => $projectCountyTotals,
        'projectSectorTotals'  => $projectSectorTotals,
        'joinedData'           => $joinedData,
        'filterCounties'       => $filterCounties,
        'filterConstituencies' => $filterConstituencies,
        'filterWards'          => $filterWards,
